...
const mergeExcludeDates = (trashExcludes: ExcludeDate[] | undefined, globalExcludes: ExcludeDate[]): ExcludeDate[] => {
...
const applyGlobalExcludes = (trashes: TrashData[], globalExcludes: ExcludeDate[]): TrashData[] => {
    ...
    calculateNextDateBySchedule(today: Date, schedule_type: string, schedule_val: string | EvweekValue, excludes: ExcludeDate[]): Date {
        if(schedule_type === "weekday") {
            const getRecentlyDate = (base_dt: Date): Date => {
                ...
                let next_date = diff_day < 0 ? base_dt.getDate() + (7 + diff_day) : base_dt.getDate() + diff_day;
                recently_dt.setDate(next_date);
                // next_dateがrecently_dtが示す年月の日数を超えている場合、月を跨いでいるため自動調整された日付を再設定する
                next_date = recently_dt.getDate();
