...
- [『今日のゴミ出し』について](#%e4%bb%8a%e6%97%a5%e3%81%ae%e3%82%b4%e3%83%9f%e5%87%ba%e3%81%97%e3%81%ab%e3%81%a4%e3%81%84%e3%81%a6)
  - [ゴミ出し予定の登録](#%e3%82%b4%e3%83%9f%e5%87%ba%e3%81%97%e4%ba%88%e5%ae%9a%e3%81%ae%e7%99%bb%e9%8c%b2)
    ...
    - [例外日の設定](#%e4%be%8b%e5%a4%96%e6%97%a5%e3%81%ae%e8%a8%ad%e5%ae%9a)
...
登録画面は以下の3画面で構成されています。

- **ゴミの種類一覧画面**: ゴミの種類一覧、共通例外日設定へのボタン、登録ボタンを表示します。ゴミの種類追加はここから行います。
- **ゴミだしスケジュール編集画面**: ゴミの種類とスケジュールを編集します。設定ボタンで一覧に戻ります。
- **共通例外日設定画面**: 全体の例外日を設定し、設定後は一覧に戻ります。
...
ゴミ出しの対象外にしたい日付を設定できます。設定は以下の2種類です。

- **全体の例外日**: すべてのゴミに共通で適用されます。ゴミの種類一覧画面の **「全体の例外日を設定」** から登録できます（最大10件）。一覧画面ではボタン下に設定済みの全体例外日をチップで要約表示するため、今の設定を見ながらスケジュールを編集できます。
- **個別の例外日**: ゴミの種類ごとに適用されます。各ゴミの **「例外日を設定」** から登録します（最大10件）。

全体例外日・個別例外日のいずれかに該当する場合、その日はゴミ出し対象外になります。
